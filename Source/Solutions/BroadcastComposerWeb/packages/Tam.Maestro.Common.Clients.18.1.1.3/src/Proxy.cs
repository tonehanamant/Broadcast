...
namespace Tam.Maestro.Services.Clients
{
        ...
        where T : ITAMService
    {
        ...
        public Proxy(TAMService tamService, object pImplementation, string pUri)
        {
            ...
            this.Init();
            SMSClient.Handler.UriChanged += new GenericEvent<TAMService, string>(this.OnUriChanged);
        }
        ...
        private void OnUriChanged(TAMService pTAMService, string pNewUri)
        {
            if (this._TAMService == pTAMService)
            {
                lock (this._ChannelFactory)
                {
                    CloseOrAbortClientChannel(this._ChannelFactory);
                    this._Uri = new Uri(pNewUri);
                    this.Init();
                }
            }
        }
