...
IF NOT EXISTS (SELECT * FROM sys.types WHERE is_table_type = 1 AND name = 'ProgramMappingUpdateRequests')
BEGIN
	EXEC('CREATE TYPE [dbo].[ProgramMappingUpdateRequests] AS TABLE(
...
IF EXISTS(SELECT 1 FROM sysobjects WHERE id = object_id(N'[dbo].[usp_UpdateProgramNameMappings]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
BEGIN
	DROP PROCEDURE [dbo].[usp_UpdateProgramNameMappings]
END
