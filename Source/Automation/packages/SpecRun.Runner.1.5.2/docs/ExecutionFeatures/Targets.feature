...
@server
Scenario: If current executing target switches, log message are not printed multiple times
	Given I have a feature file with a scenario as
		"""
		Feature: My Feature
		Scenario: Scenario 1
		...
		Scenario: Scenario 3
			When I do something
		"""
	And there is a SpecRun server running
	And the following test run result is published to the server
		| Target | Feature    | Scenario   | Result    |
		| T1     | My Feature | Scenario 1 | Succeeded |
		| T1     | My Feature | Scenario 2 | Succeeded |
		| T1     | My Feature | Scenario 3 | Failed    |
		| T2     | My Feature | Scenario 1 | Succeeded |
		| T2     | My Feature | Scenario 2 | Succeeded |
		| T2     | My Feature | Scenario 3 | Failed    |
	And all steps are bound and pass
	...
	And there is a specrun configuration file 'Default.srprofile' as
		...
		<?xml version="1.0" encoding="utf-8"?>
		<TestProfile xmlns="http://www.specrun.com/schemas/2011/09/TestProfile">
			<Settings projectName="SpecRun.TestProject" projectId="{7DFCB452-AC06-4D2C-ADDA-8C62A1BDD36A}" />
			<Execution stopAfterFailures="0" testThreadCount="1" testSchedulingMode="Adaptive" retryCount="0" />
			<TestAssemblyPaths>
				<TestAssemblyPath>SpecRun.TestProject.dll</TestAssemblyPath>
			</TestAssemblyPaths>
			<Server serverUrl="http://localhost:8995" publishResults="false" />
			<Targets>
				<Target name="T1" />
				<Target name="T2" />
			</Targets>
        ...
        """
	When I execute the tests through the console runner
	Then the console runner log should contain 'Thread#0:B:When I do something' '6' times
