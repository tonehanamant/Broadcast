...
*****************************************************************************************************/
CREATE FUNCTION [dbo].[udf_GetMarriedTrafficAmountByTrafficIdAndProposalID]
(
      @traffic_id INT,
	  @proposal_id INT
)
...
BEGIN
      DECLARE @return AS MONEY;
		 ...
		 ( SELECT (case pp.ordinal when 0 then ISNULL(td.traffic_amount1,0) else  ISNULL(td.traffic_amount2,0) end)  * (pp.rotation_percentage / 100.0) as amount
