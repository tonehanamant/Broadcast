	...
	-- Description:	
	-- =============================================
	CREATE FUNCTION [dbo].[udf_ExpandZoneGroupsAsOf]
	(
	...
	(
		...
		zone_group_zones(
			...
			select	
				...
				zone_zone.type
			from
				udf_GetZoneZonesAsOf(@dateAsOf, 'ZoneGroup') zone_zone
			union all
			select
			...
			from
				zone_group_zones
				join udf_GetZoneZonesAsOf(@dateAsOf, 'ZoneGroup') zone_zone on
