...
-- Description:	Gets the zones in all of the topographies identified by the passed in table of topography ids
-- =============================================
CREATE FUNCTION [dbo].[udf_GetZonesInTopographiesByDate]
(	
			...
			(
				...
				z.zone_id IN (
					...
					FROM 
						...
						INNER JOIN @topographyIds ti on tzu.topography_id = ti.id
					WHERE 
					...
					WHERE 
						...
						AND zbu.business_id IN (
							...
							FROM 
								...
								INNER JOIN @topographyIds ti ON tbu.topography_id = ti.id
							WHERE 
					...
					WHERE 
						...
						AND zdu.dma_id IN (
							...
							FROM 
								...
								INNER JOIN @topographyIds ti ON tdu.topography_id = ti.id
							WHERE 
					...
					WHERE 
						...
						AND zsu.state_id IN (
							...
							FROM 
								...
								INNER JOIN @topographyIds ti on tsu.topography_id = ti.id
							WHERE 
							...
							WHERE
								sgsu.system_group_id IN (
									...
									FROM 
										...
										INNER JOIN @topographyIds ti on tsgu.topography_id = ti.id
									WHERE 
