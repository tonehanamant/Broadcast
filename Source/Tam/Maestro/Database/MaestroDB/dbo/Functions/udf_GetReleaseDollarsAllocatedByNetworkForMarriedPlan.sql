/****** Object:  Function [dbo].[udf_GetReleaseDollarsAllocatedByNetworkForMarriedPlan]    Script Date: 02/12/2014 10:16:38 ******/
CREATE FUNCTION [dbo].[udf_GetReleaseDollarsAllocatedByNetworkForMarriedPlan]
(
...
BEGIN
	...
	FROM
      ...
      SET @return = (
            SELECT 
                  CASE WHEN @source_proposal_id = @proposal_id1 THEN
                        CASE WHEN (@proposal_cpm1 + @proposal_cpm2) > 0 THEN
                              CAST(((@proposal_cpm1 / (@proposal_cpm1 + @proposal_cpm2)) * td.release_amount) * ((pp.rotation_percentage / 100.0)) AS MONEY)
                        ELSE
                              0.0
                        END
                  ELSE
                        CASE WHEN (@proposal_cpm1 + @proposal_cpm2) > 0 THEN
                              CAST(((@proposal_cpm2 / (@proposal_cpm1 + @proposal_cpm2)) * td.release_amount) * ((pp.rotation_percentage / 100.0)) AS MONEY)
                        ELSE 
                              0.0
                        END
                  END
            ...
            WHERE
				pp.child_proposal_id = @source_proposal_id 
				AND td.id = @traffic_detail_id
      )
      RETURN @return;
END
