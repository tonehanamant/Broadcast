...
-- Description:	Calculate the rotational biases using the classic 3 month average
-- =============================================
CREATE PROCEDURE [dbo].[usp_ARSLoader_CalculateRotationalBiases3Month]
...
BEGIN
	...
	FROM
		media_months
	WHERE
		dateadd(month, -1, @month) between start_date AND end_date;
	...
	FROM
		media_months
	WHERE
		dateadd(month, -2, @month) between start_date AND end_date;
	...
	rbmm(
		...
		WHERE 
			mm.id IN (@monthID, @monthIDm1, @monthIDm2)
		GROUP BY
			mm.id,
			mm.media_month,
			mm.start_date,
	...
	nn_dr(
		...
		FROM
			...
			JOIN delivered_ratings dr
				ON dr.media_month_id IN (@monthID, @monthIDm1, @monthIDm2)
