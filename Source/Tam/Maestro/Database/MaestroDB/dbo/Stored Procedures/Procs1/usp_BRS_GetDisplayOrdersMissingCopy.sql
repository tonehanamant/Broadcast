...
CREATE PROCEDURE [dbo].[usp_BRS_GetDisplayOrdersMissingCopy]
AS
BEGIN
	...
	WHERE
		cmw_traffic.id not in (select cmw_traffic_id from cmw_traffic_materials (NOLOCK))
		AND DATEDIFF(DAY,cmw_traffic.start_date, GETDATE()) >= 2
	ORDER BY 
