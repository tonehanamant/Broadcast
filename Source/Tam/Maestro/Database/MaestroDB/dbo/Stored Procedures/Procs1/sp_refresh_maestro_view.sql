...
CREATE PROCEDURE [dbo].[sp_refresh_maestro_view]
...
exec [dbo].[sp_refresh_run] 'drop current view'

if @delete <> 1
begin
		--build report view
			insert into ##TempCmd(cmd_)
				select sql_text from dba.dbo.maestro_report_view_text
					where view_type=@view_type

		--execute create specific report view commands
		exec [dbo].[sp_refresh_run] 'create specific report view'
		...
		set @string_='insert into ##TempCmd(cmd_)  
				select ''create view '' + s.name + ''.'+@view_type+'_''+ o.name + '' as select * from ''
					...
					from sys.objects o
						...
						on s.schema_id=o.schema_id
					where o.type in (''U'')
						and o.name not in (select table_name from dba.dbo.maestro_report_view_text
												where view_type='''+@view_type+''')
					and s.name =''dbo'''
		...
		exec [dbo].[sp_refresh_run] 'create report view'
		drop table ##TempCmd
