CREATE PROCEDURE [dbo].[spIndex_rebuild_r_ip]
@DB_ID int,
@frag_in_percent int,
@Date_Diff int,
@Row_Diff int
...
--	select @string='Execute tamsql02.msdb.dbo.sp_update_job @job_id = ''' + CAST(job.job_id AS CHAR(36)) + ''', @enabled = 0;' 
...
--	EXECUTE tamsql02.master.dbo.sp_drop_snapshot_all 'maestro'
-- Open the cursor.
--set @TableNameOld=''
--set @index_name_old=''
...
--	select @string='Execute tamsql02.msdb.dbo.sp_update_job @job_id = ''' + CAST(job.job_id AS CHAR(36)) + ''', @enabled = 1;' 
--		from tamsql02.msdb.dbo.sysjobs job
--			where job.[name]='load snapshot data'
--			--print @string
--		exec (@string)
