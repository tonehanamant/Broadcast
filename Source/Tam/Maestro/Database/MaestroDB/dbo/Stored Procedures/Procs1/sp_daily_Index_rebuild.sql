...
--exec sp_daily_Index_rebuild 10, 1, 100
create PROCEDURE [dbo].[sp_daily_Index_rebuild]
@frag_set int,
@Date_Diff int,
@Row_Diff int
AS
...
--delete temp table
IF OBJECT_ID('temp_db_backup.dbo.temp_daily_reindex_row_count') IS NOT NULL 
	DROP TABLE temp_db_backup.dbo.temp_daily_reindex_row_count;

select aa.table_id, i.name as index_name, aa.RowCnt
into temp_db_backup.dbo.temp_daily_reindex_row_count
...
--drop temp table
IF OBJECT_ID('temp_db_backup.dbo.temp_daily_reindex') IS NOT NULL 
	DROP TABLE temp_db_backup.dbo.temp_daily_reindex;
...
SELECT
    ...
    page_count, 
    index_type_desc
INTO temp_db_backup.dbo.temp_daily_reindex
FROM sys.dm_db_index_physical_stats (DB_ID(), NULL, NULL , NULL, 'LIMITED')
join temp_db_backup.dbo.temp_daily_reindex_row_count r
...
FOR 
SELECT top 10 * FROM temp_db_backup.dbo.temp_daily_reindex
order  by 4 desc;
...
WHILE (1=1)
    BEGIN;
        ...
        join temp_db_backup.dbo.temp_daily_reindex_row_count t
