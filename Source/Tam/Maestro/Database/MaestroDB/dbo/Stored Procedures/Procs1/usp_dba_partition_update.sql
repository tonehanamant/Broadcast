...
--exec [dbo].[usp_dba_partition_update]
create PROCEDURE [dbo].[usp_dba_partition_update]
AS
...
--	MediaMonthIDRangePFN()	
	...
	begin
		...
		values('maestro', 'partition tables', 'MediaMonthIDRangePFN', @count, 'extend partition range',  @string, getdate(), @start_dt, getdate()-@start_dt )
...
--MediaMonthSmallintPFN
	...
	begin
		...
		insert into dba.dbo.partition_key_logs(database_name, table_name, partition_function, 
			count_, events, sql_string, create_dt, min_date, max_date)
		values('maestro', 'small int', 'MediaMonthSmallintPFN', @count, 'extend small int range',  @string, getdate(), @start_dt, getdate()-@start_dt )
