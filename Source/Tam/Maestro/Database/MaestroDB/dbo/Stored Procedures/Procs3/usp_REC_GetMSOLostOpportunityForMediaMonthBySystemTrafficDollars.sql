...
CREATE Procedure [dbo].[usp_REC_GetMSOLostOpportunityForMediaMonthBySystemTrafficDollars]
(
	@media_month_id int,
	@business_id int
)
...
SET @effective_date = (select media_months.end_date from media_months where id = @media_month_id);
	...
	where 
		...
		traffic_orders.zone_id is not null and traffic_orders.release_id is not null and
