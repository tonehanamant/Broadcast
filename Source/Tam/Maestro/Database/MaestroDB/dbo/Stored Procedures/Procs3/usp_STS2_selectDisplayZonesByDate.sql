...
CREATE PROCEDURE [dbo].[usp_STS2_selectDisplayZonesByDate]
	@active bit,
...
BEGIN
	...
	(
		...
		LEFT JOIN time_zones tz on uvw_zone_universe.time_zone_id = tz.id
		WHERE 
		...
		FROM zones (NOLOCK) 
			...
			LEFT JOIN time_zones tz on zones.time_zone_id = tz.id
		WHERE 
...
/****** Object:  StoredProcedure [dbo].[usp_STS2_selectDisplayZonesForDmaByDate]    Script Date: 11/29/2016 11:42:15 AM ******/
SET ANSI_NULLS ON
