...
CREATE Procedure [dbo].[usp_REC_GetMSOLostOpportunityForYearBySystemTrafficDollars]
(
	@year int,
	@business_id int
)
...
set @end_date = (select max(media_months.end_date) from media_months where year = @year);
	...
	select systems.code, systems.id, sum(traffic_orders.ordered_spot_rate * traffic_orders.ordered_spots )
		from traffic_orders (NOLOCK)
		join traffic_details (NOLOCK) on traffic_details.id = traffic_orders.traffic_detail_id
		join traffic_detail_weeks (NOLOCK) on traffic_details.id = traffic_detail_weeks.traffic_detail_id and traffic_orders.start_date >= traffic_detail_weeks.start_date and traffic_orders.end_date <= traffic_detail_weeks.end_date 
	...
	where 
		...
		group by systems.code, systems.id
