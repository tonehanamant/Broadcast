...
** ---------	----------		--------------------------------------------------------------------
** 05/15/2015	Abdul Sukkur	Get coverage universe calculation for market details
...
*****************************************************************************************************/
CREATE PROCEDURE [dbo].[usp_CUS_GetCoverageUniversesForMarketDetails]
	  @dma_id int,
	  @network_id  int,
...
--get subscribers count for month1/month2
SELECT
			zn.network_id,
		...
		FROM 
			...
			JOIN uvw_zone_universe z (NOLOCK) ON z.zone_id=zd.zone_id
			JOIN uvw_zonenetwork_universe zn (NOLOCK) ON zn.zone_id=zd.zone_id
