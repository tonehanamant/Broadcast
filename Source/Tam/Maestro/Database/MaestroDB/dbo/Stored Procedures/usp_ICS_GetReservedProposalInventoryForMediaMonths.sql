...
-- Modified:	8/16/2016 - multiplied contracted subscribers by spot_lengths.delivery_multiplier and removed parameter @inventory_state_filter_code TINYINT
-- Description: Returns reserved inventory specific to the media months specified
-- =============================================
/*
       DECLARE @media_month_ids VARCHAR(MAX) = '418,419'
       EXEC usp_ICS_GetReservedProposalInventoryForMediaMonths @media_month_ids,NULL,0
	   EXEC usp_ICS_GetReservedProposalInventoryForMediaMonths @media_month_ids,NULL,1
*/
CREATE PROCEDURE [dbo].[usp_ICS_GetReservedProposalInventoryForMediaMonths]
       @media_month_ids VARCHAR(MAX),
       @effective_date DATETIME = NULL, -- NULL indicates current DATETIME
...
BEGIN
	...
	DECLARE @media_month_ids_split TABLE (media_month_id INT);
	INSERT INTO @media_month_ids_split
		SELECT id FROM dbo.SplitIntegers(@media_month_ids);
	...
	INSERT INTO @proposals_on_avail_planner
		...
		FROM
			...
			JOIN @media_month_ids_split mmis ON mmis.media_month_id=mw.media_month_id
		WHERE
			p.include_on_availability_planner=1
			AND p.total_gross_cost>0 -- filter out zero cost plans
			AND p.date_created<=@effective_date
		GROUP BY
			p.id,
			proposal_status_id
	...
	DECLARE @all_cpms_of_all_contracts_in_month TABLE (proposal_detail_id INT NOT NULL, cpm_eq MONEY NOT NULL, PRIMARY KEY (proposal_detail_id));
	...
	DECLARE @contract_details TABLE (proposal_id INT NOT NULL, proposal_detail_id INT NOT NULL, daypart_id INT NOT NULL, media_month_id SMALLINT NOT NULL, media_week_id INT NOT NULL, proposal_status_id INT NOT NULL, network_id INT NOT NULL, contracted_subscribers BIGINT NOT NULL, PRIMARY KEY (proposal_id, proposal_detail_id, daypart_id, media_week_id))
	...
	INSERT INTO @media_plan_competition
		SELECT
			...
			CAST(cd.proposal_detail_id AS VARCHAR) 'UniqueProposalDetailId',
		...
		FROM
			...
			JOIN @all_cpms_of_all_contracts_in_month pdcpm ON pdcpm.proposal_detail_id=cd.proposal_detail_id
			JOIN @cpm_bins b ON pdcpm.cpm_eq >= b.cpm_eq_start AND pdcpm.cpm_eq < b.cpm_eq_end
		GROUP BY
			cd.proposal_id,
			cd.proposal_detail_id,
			cd.media_month_id,
			cd.media_week_id,
			cd.network_id,
