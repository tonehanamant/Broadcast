...
-- =============================================
/*
	DECLARE @media_month_id SMALLINT = 418
	DECLARE @inventory_flight_selection_code TINYINT = 1
	EXEC usp_ICS_GetReservedProposalInventoryForMonthlySummary @media_month_id,@inventory_flight_selection_code,0
	EXEC usp_ICS_GetReservedProposalInventoryForMonthlySummary @media_month_id,@inventory_flight_selection_code,1
*/
CREATE PROCEDURE [dbo].[usp_ICS_GetReservedProposalInventoryForMonthlySummary]
	@media_month_id INT,
...
BEGIN
	...
	BEGIN
		...
		DELETE FROM @proposals_on_avail_planner;
		INSERT INTO @proposals_on_avail_planner
			SELECT
				p.id,
				p.proposal_status_id
			FROM
				...
				JOIN proposal_detail_worksheets pdw ON pdw.proposal_detail_id=pd.id
					AND pdw.units>0
				JOIN media_weeks mw ON mw.id=pdw.media_week_id
				JOIN @relevant_media_month_ids rmmi ON rmmi.media_month_id=mw.media_month_id
			WHERE
				p.include_on_availability_planner=1
				AND p.total_gross_cost>0 -- filter out zero cost plans
			GROUP BY
	...
	INSERT INTO @contract_details
		...
		FROM
			...
			JOIN proposal_detail_worksheets pdw ON pdw.proposal_detail_id=pd.id
			JOIN media_weeks mw ON mw.id=pdw.media_week_id
				AND (@inventory_flight_selection_code = 1 OR mw.media_month_id=@media_month_id)
		WHERE
			pdw.units>0
