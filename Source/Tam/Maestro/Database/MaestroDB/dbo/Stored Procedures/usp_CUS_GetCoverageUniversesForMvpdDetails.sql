...
** ---------	----------		--------------------------------------------------------------------
** 05/15/2015	Abdul Sukkur	Get coverage universe calculation for Mvpd details
...
*****************************************************************************************************/
CREATE PROCEDURE [dbo].[usp_CUS_GetCoverageUniversesForMvpdDetails]
      @mvpd_id int,
	  @network_id  int,
	  @month2_network_id  int,
...
Select distinct zone_network_id 
from udf_GetTrafficNetworkZoneNetworkMap(@effective_date_media_month2)  where traffic_network_id = @month2_network_id
insert into @NetWorkList2 values (@month2_network_id)
...
SELECT
		...
		FROM 
			...
			INNER JOIN uvw_zonenetwork_universe zn (NOLOCK) ON zn.zone_id=z.zone_id
			INNER JOIN uvw_network_universe n (NOLOCK) ON n.network_id=zn.network_id 
			INNER JOIN uvw_zonebusiness_universe zb (NOLOCK) ON zb.zone_id = z.zone_id AND zb.[type] ='MANAGEDBY'
			INNER JOIN uvw_business_universe b (NOLOCK) ON b.business_id=zb.business_id AND b.business_id = @mvpd_id
		WHERE 
		    b.business_id = @mvpd_id 
			AND n.network_id in (select NetworkId from @NetWorkList1)
			AND z.[active] = 1 
			--AND z.[primary] = 1 
			--AND zn.[primary] = 1 
			AND z.[traffic] = 1 
			...
			group by zn.network_id, n.code, z.zone_id, z.[primary],zn.[primary], z.name, z.[type], z.[code] ;
...
SELECT
		...
		WHERE 
			b.business_id = @mvpd_id 
			AND n.network_id  in (select NetworkId from @NetWorkList2)
			AND z.[active] = 1 
			--AND z.[primary] = 1 
			--AND zn.[primary] = 1 
			AND z.[traffic] = 1 
			...
			AND (zb.[start_date] <=@effective_date_media_month2 AND (zb.[end_date] >=@effective_date_media_month2 OR zb.[end_date]  IS NULL))
			AND (b.[start_date] <=@effective_date_media_month2 AND (b.[end_date] >=@effective_date_media_month2 OR b.[end_date]  IS NULL))
...
INTO #OWNED_BY
	FROM 
	#ZONE_LIST zl (NOLOCK) 
	INNER JOIN uvw_zonenetwork_universe zn (NOLOCK) ON zn.zone_id = zl.zone_id AND zn.network_id in (select NetworkId from @NetWorkList1)
	INNER JOIN uvw_zone_universe z (NOLOCK) ON zn.zone_id=z.zone_id
	INNER JOIN uvw_network_universe n (NOLOCK) ON n.network_id=zn.network_id 
	INNER JOIN uvw_zonebusiness_universe zb1 (NOLOCK) ON zb1.zone_id = zn.zone_id
		AND zb1.[type] ='OWNEDBY' 
		AND zb1.business_id = @mvpd_id
	INNER JOIN uvw_zonebusiness_universe zb2 (NOLOCK) ON zb2.zone_id = zn.zone_id
		AND zb2.[type] ='MANAGEDBY'  AND zb1.business_id = zb2.business_id AND zb1.business_id = @mvpd_id
	INNER JOIN uvw_business_universe b (NOLOCK) ON b.business_id=zb1.business_id
	WHERE 
	    b.business_id = @mvpd_id 
		AND n.network_id  in (select NetworkId from @NetWorkList1)
		AND z.[active] = 1 
		...
		AND (z.[start_date] <=@effective_date_media_month1 AND (z.[end_date] >=@effective_date_media_month1 OR z.[end_date]  IS NULL)) 
		AND (zn.[start_date] <=@effective_date_media_month1 AND (zn.[end_date] >=@effective_date_media_month1 OR zn.[end_date]  IS NULL))
		AND (zb1.[start_date] <=@effective_date_media_month1 AND (zb1.[end_date] >=@effective_date_media_month1 OR zb1.[end_date]  IS NULL))
		AND (zb2.[start_date] <=@effective_date_media_month1 AND (zb2.[end_date] >=@effective_date_media_month1 OR zb2.[end_date]  IS NULL))
		AND (b.[start_date] <=@effective_date_media_month1 AND (b.[end_date] >=@effective_date_media_month1 OR b.[end_date]  IS NULL))
		AND (n.[start_date] <=@effective_date_media_month1 AND (n.[end_date] >=@effective_date_media_month1 OR n.[end_date]  IS NULL))
group by zn.zone_id
...
SELECT  distinct
		...
		FROM 
			#ZONE_LIST zl (NOLOCK) 
			LEFT JOIN #OWNED_BY own (NOLOCK) on own.zone_id = zl.zone_id 
			...
			LEFT JOIN uvw_dma_universe (NOLOCK) d ON d.dma_id= zd.dma_id 
