...
--exec [dba].[spIndex_rebuild_daily_list] 'dba', 0, 20, 0, 0, 5
--select * from dbo.daily_reindex_list
--select * from dbo.reindex_daily_row_count_daily
--truncate table dbo.daily_reindex_list
CREATE PROCEDURE [dba].[spIndex_rebuild_daily_list]
@dbname_ varchar(50),
@priority_type int,
@frag_set int,
@Date_Diff int,
...
insert into dba.dbo.reindex_daily_row_count_daily([db_name], table_id, index_name, RowCnt)
select @dbname_, aa.table_id, i.name as index_name, aa.RowCnt
...
join dba.dbo.reindex_daily_row_count_daily r
...
--FOR 
--SELECT * FROM temp_db_backup.dbo.temp_reindex_daily1;
...
--        join temp_db_backup.dbo.temp_reindex_daily_row_count_daily t
