 ...
 * */
const isRenderMenuItem = (isShow, rowDetail) => ((isShow && rowDetail) ? isShow(rowDetail) : !!rowDetail);

const filterMenuItems = rowDetail => ({ isShow }) => (isRenderMenuItem(isShow, rowDetail));
...
const generateMenuItems = rowDetail => (
  flow([
    filter(filterMenuItems(rowDetail)),
    map(mapMenuItems),
  ])
);
