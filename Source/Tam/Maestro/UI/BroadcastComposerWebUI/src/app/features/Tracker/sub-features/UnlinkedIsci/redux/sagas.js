...
import { actions as trackerActions } from "Tracker/redux";
import api from "API";
import {
...
export const selectFilteredIscis = state =>
  state.tracker.unlinkedIsci.unlinkedFilteredIscis;
export const selectActiveIsciFilter = state =>
  state.tracker.unlinkedIsci.activeIsciFilterQuery;
...
export function* closeUnlinkedIsciModal({ modalPrams }) {
  ...
  yield put(trackerActions.getTracker());
}
