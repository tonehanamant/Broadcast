...
//allowing for any time (not set to hours) so adjust accordingly

//issue when popover is inside modals - input focus can be blocked; need to change the container setting to the containing view; adding param popoverContainer
			...
			$.extend(Plugin.prototype, {
					init: function () {
							...
							$(this.element).webuiPopover({
									container: _this.settings.popoverContainer,
									//selector: '#new_program_airtime_input',
									arrow: false,
									multiple: true,
									animation: 'pop',
									style: 'custom',
					...
					//GU: there is no record in form only implementations; deal with destroying
					handleKeyDown: function (event) {
							if (event.keyCode === 9) {//TAB
									var _this = this;
									event.preventDefault(); //this._onClosePopup();
									this.destroy();
									//what to do if no record?
					...
					generateDateObject: function () {
							...
							var dateValue = {
									...
									toString: function () { return dateToString(this); }
							};
