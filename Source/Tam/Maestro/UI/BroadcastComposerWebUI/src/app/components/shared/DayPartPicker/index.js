...
const dateToString = dateObj => {
  ...
  while (currentDay <= 6) {
    if (dateObj[days[currentDay]]) {
      ...
      if (endDay > startDay) {
        daysBuilder.push(
          `${aux + dayAbbreviationStrings[startDay]}-${
            dayAbbreviationStrings[endDay]
          }`
        );
      } else {
  ...
  builder.push(
    `${timeToString(dateObj.StartTime)}-${timeToString(dateObj.EndTime)}`
  );
  return builder.join(" ");
};
...
export default class DayPartPicker extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      startTime: moment()
        .startOf("day")
        .add(this.props.dayPart.startTime, "seconds"),
      startTimeSeconds: this.props.dayPart.startTime,
      endTime: moment()
        .startOf("day")
        .add(this.props.dayPart.endTime, "seconds"),
  ...
  onEndTimeChange(time, timeString) {
    const timeSeconds = moment(time).diff(
    ...
    return (
      ...
      >
        <FormGroup validationState={this.props.validationState}>
          <ControlLabel style={{ margin: "0 10px 0 16px" }}>
            Daypart
          </ControlLabel>
                            ...
                            <Row>
                              <Col md={4}>
                                <Radio
                                  ...
                                  onChange={() => {
                                    this.onQuickOptionSelect("everyday");
                                  }}
                                >
                                  Everyday
                                </Radio>
                              ...
                              <Col md={4}>
                                <Radio
                                  ...
                                  onChange={() => {
                                    this.onQuickOptionSelect("weekdays");
                                  }}
                                >
                                  Weekdays
                                </Radio>
                              ...
                              <Col md={4}>
                                <Radio
                                  ...
                                  onChange={() => {
                                    this.onQuickOptionSelect("weekends");
                                  }}
                                >
                                  Weekends
                                </Radio>
