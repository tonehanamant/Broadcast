...
import { setOverlayProcessing, createAlert, setOverlayLoading, toggleModal } from 'Ducks/app';
import { selectModal } from 'Ducks/app/selectors';
import { selectActiveScrubs, selectActiveFilterKey } from 'Ducks/tracker/selectors';
...
export function* requestTrackerFiltered({ payload: query }) {
  const trackerListUnfiltered = yield select(state => state.tracker.trackerUnfilteredGridData);

  const keys = ['searchContractId', 'ContractName', 'Advertiser', 'searchUploadDate', 'searchSpotsInSpec', 'searchSpotsOutOfSpec'];
  const searcher = new FuzzySearch(trackerListUnfiltered, keys, { caseSensitive: false });
...
export function* requestUnlinkedFiltered({ payload: query }) {
  ...
  // const unlinkedListUnfiltered = yield select(state => state.tracker.archivedFilteredIscis);
...
export function* requestClearScrubbingDataFiltersList() {
  try {
    yield put({
      type: ACTIONS.TRACKER_RECEIVE_CLEAR_SCRUBBING_FILTERS_LIST,
      data: [],
    });
  } catch (e) {
    if (e.message) {
...
export function* requestScrubbingDataFiltered({ payload: query }) {
  const listUnfiltered = yield select(state => state.tracker.proposalHeader.scrubbingData.ClientScrubs);
  const listFiltered = yield select(state => state.tracker.proposalHeader.activeScrubbingData.ClientScrubs);
  const activeFilters = _.cloneDeep(yield select(state => state.tracker.activeScrubbingFilters));
  const originalFilters = yield select(state => state.tracker.activeScrubbingFilters);
  ...
  try {
    ...
    yield put({
      type: ACTIONS.TRACKER_RECEIVE_FILTERED_SCRUBBING_DATA,
      data: filtered,
    });
  } catch (e) {
    if (e.message) {
...
export function* requestOverrideStatus({ payload: params }) {
  ...
  try {
    ...
    const { status, data } = response;
    const hasActiveScrubbingFilters = yield select(state => state.tracker.hasActiveScrubbingFilters);
    yield put({
      type: ACTIONS.SET_OVERLAY_LOADING,
      overlay: {
        id: 'trackerOverrideStatus',
    ...
    if (hasActiveScrubbingFilters) {
      const scrubs = yield select(state => state.tracker.proposalHeader.activeScrubbingData.ClientScrubs);
      ...
      data.Data.filterKey = yield select(state => state.tracker.activeFilterKey);
      yield put({
        type: ACTIONS.RECEIVE_TRACKER_CLIENT_SCRUBBING,
        data,
      });
...
export function* swapProposalDetail({ payload: params }) {
  ...
  try {
    ...
    // refresh scrubbing
    const id = yield select(state => state.tracker.proposalHeader.activeScrubbingData.Id);
...
export function* watchRequestTrackerClientScrubbing() {
  yield takeEvery(ACTIONS.REQUEST_TRACKER_CLIENT_SCRUBBING, requestTrackerClientScrubbing);
}

export function* watchRequestScrubbingDataFiltered() {
  yield takeEvery(ACTIONS.TRACKER_REQUEST_FILTERED_SCRUBBING_DATA, requestScrubbingDataFiltered);
}

export function* watchRequestClearScrubbingFiltersList() {
  yield takeEvery(ACTIONS.TRACKER_REQUEST_CLEAR_SCRUBBING_FILTERS_LIST, requestClearScrubbingDataFiltersList);
}
...
export function* watchRequestOverrideStatus() {
  yield takeEvery(ACTIONS.REQUEST_TRACKER_OVERRIDE_STATUS, requestOverrideStatus);
}

export function* watchSwapProposalDetail() {
  yield takeEvery(ACTIONS.TRACKER_REQUEST_SWAP_PROPOSAL_DETAIL, swapProposalDetail);
}
...
export function* watchRescrubUnlinkedIsci() {
  yield takeEvery(ACTIONS.TRACKER_RESCRUB_UNLIKED_ISCI.request, sagaWrapper(rescrubUnlinkedIsci, ACTIONS.RESCRTRACKER_RESCRUB_UNLIKED_ISCIUB_UNLIKED_ISCI));
}
...
export function* watchUndoScrubStatus() {
  yield takeEvery(ACTIONS.TRACKER_UNDO_SCRUB_STATUS.request, sagaWrapper(undoScrubStatus, ACTIONS.TRACKER_UNDO_SCRUB_STATUS));
}

export function* watchUndoScrubStatusSuccess() {
  yield takeEvery(ACTIONS.TRACKER_UNDO_SCRUB_STATUS.success, undoScrubStatusSuccess);
}
