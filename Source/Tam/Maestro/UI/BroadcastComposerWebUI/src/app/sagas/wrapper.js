...
const defaultError = {
    error: 'Error',
    message: 'The server encountered an error processing the request. Please try again or contact your administrator to review error logs.',
};

export function* errorBuilder(customError, customMessage) {
    const sagaError = customError || defaultError;
    const displayError = customMessage ? { ...sagaError, message: customMessage } : sagaError;
...
function* wrapper(fn, payload, actions, customError) {
  try {
    ...
    if (status !== 200 || !data.Success) {
        yield call(errorBuilder, customError, data.Message ? data.Message : undefined);
    } else {
