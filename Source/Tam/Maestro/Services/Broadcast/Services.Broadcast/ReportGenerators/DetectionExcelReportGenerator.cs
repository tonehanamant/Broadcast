...
{
    public class DetectionExcelReportGenerator
    {
        private ScheduleReportDto _ScheduleReportDto;
        private Image _Logo;
        private const string DateFormat = "MM/dd/yyyy";

        public DetectionExcelReportGenerator(ScheduleReportDto scheduleReportDto, Image logo)
        {
        ...
        private void _GenerateBroadcastWeeksDataSheets(ExcelPackage package, bool includeDateColumnsInSpec, bool includeDateColumnOutOfSpec)
        {
            ...
            foreach (var weeklyData in _ScheduleReportDto.WeeklyData.ReportDataByWeek)
            {
                ...
                _SetWeeklyOutSpecTab(wsOutofSpec, weeklyData, includeDateColumnOutOfSpec);
            }
        }
        ...
        private void _SetWeeklyInSpecTab(ExcelWorksheet ws, WeeklyImpressionAndDeliveryDto weeklyData, bool includeDateColumns)
        {
            ...
            var rowOffset = 2;
            columnOffset = 1;
            foreach (DetectionReportData row in reportData)
            {
                ws.Cells[rowOffset, columnOffset++].Value = row.Rank == 0 ? string.Empty : row.Rank.ToString();
                ws.Cells[rowOffset, columnOffset++].Value = row.Market;
                ws.Cells[rowOffset, columnOffset++].Value = row.Station;
                ...
                {
                    ws.Cells[rowOffset, columnOffset++].Value = row.DetectionDate.ToString(DateFormat);
        ...
        private void _SetAudienceData(ExcelWorksheet ws, int rowOffset, int columnOffset, IEnumerable<DetectionReportData> reportData, bool isOutOfSpect)
        {
            var originalOffset = columnOffset;
            foreach (var reportRow in reportData)
            {
