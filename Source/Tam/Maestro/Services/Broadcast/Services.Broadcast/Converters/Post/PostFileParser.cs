...
namespace Services.Broadcast.Converters.Post
{
    public class PostFileParser : BasePostFileParser
    {
        ...
        {
            _LogInfo("Beginning parse...");
            var allParsingExceptions = new List<string>();
            var worksheet = package.Workbook.Worksheets.First();
            var headers = new Dictionary<string, int>();
                        
            _LogInfo("Beginning parsing headers...");
            ...
            _LogInfo("Completed parsing headers.");

            if (allParsingExceptions.Any())
            {
                var pex = new PostParsingException("Failed to parse header.\n", allParsingExceptions);
                _LogError($"Error parsing file headers.", pex);
                throw pex;
            }
            ...
            _LogInfo("Beginning to parse lines.");
            ...
            for (var row = 2; row <= worksheet.Dimension.End.Row; row++)
            {
                ...
                {
                    var msg = $"Row {row} has an unknown parsing issue.";
                    allParsingExceptions.Add($"\n{msg}");
                    _LogError(msg, ex);
                }
                parsedRowCount++;
            }
            ...
            if (allParsingExceptions.Any())
            {
                var pex = new PostParsingException("Following lines were not imported due to data validation issues.  Please resolve the data and upload the file again. :", allParsingExceptions);
                _LogError($"Error parsing file.", pex);
                throw pex;
            }
            else
            {
                _LogInfo($"Finished parsing {parsedRowCount}.");
            }
